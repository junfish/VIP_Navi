...
class MultiSceneDataset(Dataset):
    def __init__(self, proj_path, metadata_path, mode, transform, num_val = 100):
        ...
        for i, line in enumerate(self.lines):
            splits = line.split()
            filename = splits[0].replace(",", "")
            scene = int(splits[1].strip().replace(",", ""))
            poses = splits[2:]
            poses = list(map(lambda x: float(x.replace(",", "")), poses))
            filename = os.path.join(self.proj_path, filename)

            if self.mode == 'train':
                # if i > num_val:
                self.train_filenames.append(filename)
                self.train_scenes.append(scene)
                self.train_poses.append(poses)
            elif self.mode == 'test':
                self.test_filenames.append(filename)
                self.test_scenes.append(scene)
                self.test_poses.append(poses)
            elif self.mode == 'val':
                self.test_filenames.append(filename)
                self.test_scenes.append(scene)
                self.test_poses.append(poses)
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            image = Image.open(self.test_filenames[index])
            scene = self.test_scenes[index]
            pose = self.test_poses[index]

        return {"image": self.transform(image), "scene": scene, "pose": torch.Tensor(pose)}
...
def get_loader(model, proj_path, metadata_path, mode, geometric, batch_size, is_shuffle = False, multi_scene = False, num_val = 100):

    # Predefine image size
    if model == 'Googlenet':
        img_size = 300
        img_crop = 299
    elif model in ['Resnet', 'Resnet34', 'Resnet50', 'Resnet101', 'Resnet34Simple', 'MobilenetV3', 'Resnet34lstm', 'MobilenetV3lstm', 'Resnet34hourglass', "MobilenetV3hourglass", "Branchresnet34", "BranchmobilenetV3"]:
        img_size = 256
        img_crop = 224
    else:
        img_size = 256
    ...
    if mode == 'train':
        ...
        if geometric:
            ...
            datasets = {'train': MultiSceneDataset(proj_path, metadata_path, 'train', transform, num_val),
                        'val': MultiSceneDataset(proj_path, metadata_path, 'val', transform, num_val)}
            # data_loaders = {x: DataLoader(datasets[x], batch_size, is_shuffle, num_workers=batch_size)
            #                 for x in ['train', 'val']}
